...
def get_usd_price_feed_for_asset_symbol(symbol, chain, network):
    ...
    try:
        ...
        raise ChainlinkMissingChainError(f"Missing Chainlink mapping for chain: {chain}") from e
    ...
    if not feed_data:
        raise ChainlinkMissingAssetError(f"Missing Chainlink USD price feed for asset: {symbol}")

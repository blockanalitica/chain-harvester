...
from .exceptions import (
    ChainlinkMissingAssetError,
    ChainlinkMissingChainError,
    ChainlinkMissingMappingError,
)
...
def get_usd_price_feed_for_asset_symbol(symbol, chain, network):
    ...
    try:
        module = importlib.import_module(
            f".chainlink.mappings.{chain}", package="chain_harvester"
        )
    except ModuleNotFoundError as e:
        raise ChainlinkMissingChainError(
            f"Missing Chainlink mapping for chain: {chain}"
        ) from e
    ...
    try:
        ...
        raise ChainlinkMissingMappingError(
            f"Missing Chainlink mapping for network: {network}"
        ) from e
    ...
    if not feed_data:
        raise ChainlinkMissingAssetError(
            f"Missing Chainlink USD price feed for asset: {symbol}"
        )
